...
async function request(path, options = {}, token) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 && token && token !== SUPABASE_ANON_KEY) {
...
export async function listQuotes(token) {
  const primary = '/rest/v1/quote_requests?select=id,name,email,project_type,area,message,status,attachment_paths,created_at,updated_at&order=created_at.desc&limit=500';
  try {
    return await request(primary, {}, token);
  } catch (error) {
    const schemaIssue = /schema cache|column .* does not exist|could not find|attachment_paths|status/i.test(error?.message || '');
    if (!schemaIssue) throw error;
  }

  try {
    const rows = await request('/rest/v1/quote_requests?select=id,name,email,project_type,area,message,attachment_paths,created_at&order=created_at.desc&limit=500', {}, token);
    return rows.map(row => ({...row, status: 'new'}));
  } catch (error) {
    const schemaIssue = /schema cache|column .* does not exist|could not find|attachment_paths/i.test(error?.message || '');
    if (!schemaIssue) throw error;
    const rows = await request('/rest/v1/quote_requests?select=id,name,email,project_type,area,message,created_at&order=created_at.desc&limit=500', {}, token);
    return rows.map(row => ({...row, status: 'new', attachment_paths: []}));
  }
}
