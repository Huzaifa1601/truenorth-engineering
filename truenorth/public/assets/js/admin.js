import {createAttachmentUrl, deleteQuote, listAnalytics, listQuotes, signIn, signOut, updateQuoteStatus} from './modules/supabase.js';
...
const fileName = value => String(value || '').split('/').pop()?.replace(/^[0-9a-f-]+-/i, '') || 'Attachment';
const attachments = row => Array.isArray(row.attachment_paths) ? row.attachment_paths.filter(Boolean) : [];
...
function renderQuotes() {
  ...
  list.innerHTML = rows.map(row => { const files = attachments(row); const fileButtons = files.length ? '<div class=\"attachment-list\">' + files.map((path,index) => '<button class=\"attachment-link\" type=\"button\" data-attachment=\"' + index + '\" title=\"Open ' + escape(fileName(path)) + '\">📎 ' + escape(fileName(path)) + '</button>').join('') + '</div>' : '<span class=\"no-attachments\">No attachments</span>'; return '<article class=\"crm-card\" data-quote-id=\"' + escape(row.id) + '\"><div><h3>' + escape(row.name) + '</h3><a href=\"mailto:' + escape(row.email) + '\">' + escape(row.email) + '</a></div><div><p><strong>' + escape(row.project_type) + '</strong></p><p>' + (row.area ? escape(row.area) + ' sq ft' : 'Area not provided') + ' · ' + formatDate(row.created_at) + '</p></div><p class=\"message\" title=\"' + escape(row.message) + '\">' + escape(row.message) + '</p><div class=\"attachments-cell\">' + fileButtons + '</div><div class=\"crm-actions\"><select class=\"status-select\" data-status aria-label=\"Change quote status\"><option value=\"new\" ' + (row.status === 'new' ? 'selected' : '') + '>New</option><option value=\"quoted\" ' + (row.status === 'quoted' ? 'selected' : '') + '>Quoted</option><option value=\"won\" ' + (row.status === 'won' ? 'selected' : '') + '>Won</option><option value=\"lost\" ' + (row.status === 'lost' ? 'selected' : '') + '>Lost</option></select><button class=\"copy-button\" type=\"button\" data-copy title=\"Copy email\" aria-label=\"Copy email\">⧉</button><button class=\"delete-button\" type=\"button\" data-delete title=\"Delete request\" aria-label=\"Delete request\">×</button></div></article>'; }).join('');
}
...
function exportCsv() { const headers = ['Name','Email','Project type','Area','Status','Attachments','Message','Created']; const body = filteredQuotes().map(row => [row.name,row.email,row.project_type,row.area,row.status,attachments(row).join(' | '),row.message,row.created_at].map(value => '\"' + String(value == null ? '' : value).replaceAll('\"','\"\"') + '\"').join(',')); const blob = new Blob([[headers.join(','),...body].join('\\n')], {type:'text/csv;charset=utf-8'}); const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = 'true-north-quotes-' + new Date().toISOString().slice(0,10) + '.csv'; link.click(); URL.revokeObjectURL(link.href); toast('CSV export downloaded.'); }

async function load() {
  if (!token()) return;
  setBusy(true);
  $('[data-dashboard-status]').textContent = 'Refreshing pipeline...';
  try {
    quotes = await listQuotes(token());
    analytics = await listAnalytics(token()).catch(() => []);
    fillFilters();
    renderQuotes();
    renderAnalytics();
    $('[data-dashboard-status]').textContent = 'Updated just now · ' + quotes.length + ' total request' + (quotes.length === 1 ? '' : 's');
  } catch (error) {
    const errorMessage = error?.message || 'Could not load the dashboard.';
    const sessionExpired = /session has expired|jwt expired|invalid jwt|not authenticated/i.test(errorMessage);
    if (sessionExpired) {
      clearSession();
      clearInterval(refreshTimer);
      dashboard.hidden = true;
      loginPanel.hidden = false;
      message('Your session expired. Please sign in again.', 'error');
    } else {
      $('[data-dashboard-status]').textContent = 'Refresh failed. Your current data is still shown.';
...
async function changeStatus(row, value) { try { await updateQuoteStatus(row.id, value, token()); row.status = value; renderAnalytics(); toast('Marked ' + row.name + ' as ' + value + '.'); } catch (error) { const needsMigration = /status.*schema cache|status.*column|Could not find/i.test(error.message || ''); toast(needsMigration ? 'Status column is missing in Supabase. Run supabase/migrations/003_quote_status_repair.sql, then refresh.' : (error.message || 'Status could not be saved.'), 'error'); renderQuotes(); } }

loginForm.addEventListener('submit', async event => { event.preventDefault(); const button = loginForm.querySelector('button'); button.disabled = true; message('Signing in...'); try { const values = Object.fromEntries(new FormData(loginForm).entries()); const auth = await signIn(values.email, values.password); saveSession(auth); loginPanel.hidden = true; dashboard.hidden = false; toast('Welcome back.'); clearInterval(refreshTimer); await load(); refreshTimer = setInterval(load, 30000); } catch (error) { message(error.message || 'Unable to sign in.', 'error'); } finally { button.disabled = false; } });
...
list.addEventListener('click', async event => { const card = event.target.closest('[data-quote-id]'); if (!card) return; const row = quotes.find(item => item.id === card.dataset.quoteId); if (!row) return; const attachmentButton = event.target.closest('[data-attachment]'); if (attachmentButton) { try { attachmentButton.disabled = true; const path = attachments(row)[Number(attachmentButton.dataset.attachment)]; const url = await createAttachmentUrl(path, token()); if (!url) throw new Error('Could not create attachment link.'); window.open(url, '_blank', 'noopener,noreferrer'); toast('Attachment link opened.'); } catch (error) { toast(error.message || 'Attachment could not be opened.', 'error'); } finally { attachmentButton.disabled = false; } return; } if (event.target.closest('[data-copy]')) { try { await navigator.clipboard.writeText(row.email); toast('Email copied to clipboard.'); } catch { toast('Copy was blocked by the browser. Select the email manually.', 'error'); } } if (event.target.closest('[data-delete]')) { pendingDelete = row; $('[data-delete-dialog]').showModal(); } });
...
$('[data-cancel-delete]').addEventListener('click', () => { pendingDelete = null; $('[data-delete-dialog]').close(); }); $('[data-delete-dialog]').addEventListener('close', () => { pendingDelete = null; }); $('[data-confirm-delete]').addEventListener('click', async () => { if (!pendingDelete) return; const row = pendingDelete; try { await deleteQuote(row.id, token()); quotes = quotes.filter(item => item.id !== row.id); $('[data-delete-dialog]').close(); renderQuotes(); renderAnalytics(); toast('Quote request deleted.'); } catch (error) { toast(error.message || 'Delete failed.', 'error'); } pendingDelete = null; });
