...
For Cloudflare Workers, use the included wrangler.jsonc (assets directory: ./public). For Netlify or another static host, publish the public/ directory with no build command. For GitHub Pages, configure the deployment workflow to publish public/. The 404 page, security headers, robots file, sitemap, manifest and service worker are included.
